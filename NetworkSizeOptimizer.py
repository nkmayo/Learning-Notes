...
class myAutoenc(nn.Module):
    ...
    def __init__(self, n_features=28*28, hidden_dims=(4,4)):
        super().__init__()
        ...
        self.output = nn.Linear(dims[-1], n_features) # output same as input size
...
def training_loop(n_epochs, optimizer, model, loss_fn, train_loader):
    for epoch in range(1, n_epochs + 1):
        ...
        for imgs, _ in train_loader:
            ...
            outputs = model(imgs.view(-1,1,784))
            loss = loss_fn(outputs, imgs.view(-1,1,784))
...
with torch.no_grad(): # don't want the gradients being updated just to view an output
    outimg = model(inimg.view(1,-1).unsqueeze(0)).view(-1,28,28)

plt.imshow(outimg.permute(1,2,0))
# %% Before training example (random output)
untrained = myAutoenc()
with torch.no_grad(): # don't want the gradients being updated just to view an output
    untrainedOut = untrained(inimg.view(1,-1).unsqueeze(0)).view(-1,28,28)
